...
export const load: PageServerLoad = async ({ parent, url }) => {
	...
	try {
		...
		console.error('Error loading damaged page:', err);
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		...
		const validationResult = damagedSchema.safeParse(values);
		...
		try {
			await inventoryService.recordDamaged(validationResult.data, sessionUser.id);
		} catch (err: unknown) {
			console.error('Error logging damaged items:', err);
			const message = err instanceof Error ? err.message : String(err);

			if (message === 'INSUFFICIENT_STOCK') {
				return fail(400, {
					errors: {
						quantity: 'Current stock is insufficient to log the damaged quantity.'
					},
					values
				});
			}
		...
		throw redirect(303, '/inventory?toast=damaged_success');
	}
};
