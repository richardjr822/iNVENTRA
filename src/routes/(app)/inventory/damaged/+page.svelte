<script lang="ts">
	...
	import { damagedSchema } from '$lib/validations/inventory.schema';
	...
	function validate() {
		const result = damagedSchema.safeParse({
			product_id,
		...
		// Client-side validation: Damaged items must be within current stock
		if (selectedProduct && Number(quantity) > selectedProduct.quantity) {
			errors.quantity = `Damaged count cannot exceed current stock of ${selectedProduct.quantity} units.`;
			return false;
		}
...
<svelte:head>
	<title>Record Damaged Stock - Inventra Inventory Management</title>
	<meta name="description" content="Report and write-off damaged products from inventory." />
</svelte:head>
...
<div class="max-w-2xl mx-auto space-y-6">
	...
	<!-- Header -->
	<div>
		<h2 class="text-2xl font-black tracking-tight">Record Damaged Items</h2>
		<p class="text-sm text-muted-foreground">
			Log damaged stock write-offs. This will reduce inventory levels and record a DAMAGED
			transaction.
		</p>
	</div>
	...
	<Card.Root class="border-border/40 bg-card/65 backdrop-blur-md shadow-xl">
		<Card.Header class="pb-4">
			<Card.Title class="text-lg font-bold">Write-off Details</Card.Title>
			<Card.Description class="text-sm">
				Select a product, enter quantity written off, and note damage details for audit trails.
			</Card.Description>
		</Card.Header>
			...
			<form method="POST" use:enhance={handleSubmit} class="space-y-5">
				...
				<div class="space-y-2">
					<Label for="quantity" class="text-sm font-bold tracking-wide flex items-center gap-1">
						Quantity Damaged <span class="text-destructive font-black">*</span>
					</Label>
					<Input
						id="quantity"
						name="quantity"
				...
				<!-- Remarks Field (Damage details) -->
				<div class="space-y-2">
					<Label for="remarks" class="text-sm font-bold tracking-wide">Remarks / Damage Cause</Label
					>
					<textarea
						id="remarks"
						name="remarks"
						rows="3"
						placeholder="Describe the condition or cause (e.g., Water damage in Aisle B4, broken packaging during handling)..."
						bind:value={remarks}
						disabled={loading}
				...
				<div class="flex items-center justify-end gap-3 border-t border-border/40 pt-4">
					...
					<Button
						type="submit"
						disabled={loading || !product_id}
						class="font-bold bg-amber-500 hover:bg-amber-600 text-white flex items-center gap-1.5 cursor-pointer text-xs active:scale-95 disabled:scale-100"
					>
						{#if loading}
							<Loader2 class="h-4 w-4 animate-spin" />
							<span>Recording write-off...</span>
						{:else}
							<AlertTriangle class="h-4 w-4" />
							<span>Record Damaged Items</span>
						{/if}
					</Button>
				</div>
			</form>
