<script lang="ts">
	...
	let remarks = $state(''); // "Reason" in Zod schema
	...
	const stockDiff = $derived.by(() => {
		if (!selectedProduct || new_quantity === '') return null;
		const val = Number(new_quantity);
		if (isNaN(val)) return null;
		return val - selectedProduct.quantity;
	});

	function validate() {
		const result = adjustmentSchema.safeParse({
			product_id,
			new_quantity: new_quantity === '' ? undefined : Number(new_quantity),
	...
	$effect(() => {
		...
		if (new_quantity && errors.new_quantity) delete errors.new_quantity;
		if (remarks && errors.remarks) delete errors.remarks;
	});
	...
	$effect(() => {
		...
		if (form?.values) {
			...
			new_quantity = form.values.new_quantity !== undefined ? String(form.values.new_quantity) : '';
...
<svelte:head>
	<title>Adjust Inventory - Inventra Inventory Management</title>
	<meta name="description" content="Adjust physical stock quantities for system products." />
</svelte:head>
...
<div class="max-w-2xl mx-auto space-y-6">
	...
	<!-- Header -->
	<div>
		<h2 class="text-2xl font-black tracking-tight">Inventory Adjustment</h2>
		<p class="text-sm text-muted-foreground">
			Correct recorded stock count. This logs an ADJUSTMENT transaction and updates the snapshot.
		</p>
	</div>

	<Card.Root class="border-border/40 bg-card/65 backdrop-blur-md shadow-xl">
		<Card.Header class="pb-4">
			<Card.Title class="text-lg font-bold">Adjustment Details</Card.Title>
			<Card.Description class="text-sm">
				Select a product, input the verified physical count, and provide a detailed reason.
			</Card.Description>
		</Card.Header>
		...
		<Card.Content>
			...
			<form method="POST" use:enhance={handleSubmit} class="space-y-5">
				...
				<!-- New Quantity Input -->
				<div class="space-y-2">
					<Label for="new_quantity" class="text-sm font-bold tracking-wide flex items-center gap-1">
						New Physical Count <span class="text-destructive font-black">*</span>
					</Label>
					<Input
						id="new_quantity"
						name="new_quantity"
					...
					<!-- Delta calculation box -->
					{#if stockDiff !== null && selectedProduct}
						<div
							class="rounded-xl border border-dashed p-3 bg-muted/5 flex items-center justify-between animate-in fade-in slide-in-from-top-1 duration-150"
						>
							<span class="text-xs font-semibold text-muted-foreground"
								>Calculated Stock Change:</span
							>
							<div class="flex items-center gap-1.5 font-black text-xs">
								{#if stockDiff > 0}
									<span class="text-emerald-500 flex items-center gap-1">
										<ArrowUpRight class="h-3.5 w-3.5" />
										+{stockDiff} units (Increase)
									</span>
								{:else if stockDiff < 0}
									<span class="text-rose-500 flex items-center gap-1">
										<ArrowDownLeft class="h-3.5 w-3.5" />
										{stockDiff} units (Decrease)
									</span>
								{:else}
				...
				<!-- Remarks Field (Reason for Adjustment) -->
				<div class="space-y-2">
					<Label for="remarks" class="text-sm font-bold tracking-wide flex items-center gap-1">
						Reason for Adjustment <span class="text-destructive font-black">*</span>
					</Label>
					<textarea
						id="remarks"
						name="remarks"
						rows="3"
						placeholder="Explain why the adjustment is needed, e.g. Stock audit variance, miscount during receipt..."
						bind:value={remarks}
						disabled={loading}
				...
				<div class="flex items-center justify-end gap-3 border-t border-border/40 pt-4">
					...
					<Button
						type="submit"
						disabled={loading || !product_id}
						class="font-bold bg-primary hover:bg-primary/95 text-primary-foreground flex items-center gap-1.5 cursor-pointer text-xs active:scale-95 disabled:scale-100"
					>
						{#if loading}
