<script lang="ts">
	...
	import { ArrowLeft, Loader2, ArrowUpRight } from '@lucide/svelte';
	...
	// Find the currently selected product details to display
	const selectedProduct = $derived(data.products.find((p) => p.id === product_id));
</script>

<svelte:head>
	<title>Stock In - Inventra Inventory Management</title>
	<meta name="description" content="Add physical stock to system products." />
</svelte:head>
...
<div class="max-w-2xl mx-auto space-y-6">
	...
	<!-- Header -->
	<div>
		<h2 class="text-2xl font-black tracking-tight">Stock In</h2>
		<p class="text-sm text-muted-foreground">
			Increase product stock levels by recording incoming shipments.
		</p>
	</div>
	...
	<Card.Root class="border-border/40 bg-card/65 backdrop-blur-md shadow-xl">
		<Card.Header class="pb-4">
			...
			<Card.Description class="text-sm">
				Select a product, enter physical count received, and note reference numbers.
			</Card.Description>
		</Card.Header>
			...
			<form method="POST" use:enhance={handleSubmit} class="space-y-5">
				...
				<div class="space-y-2">
					<Label for="quantity" class="text-sm font-bold tracking-wide flex items-center gap-1">
						Quantity Received <span class="text-destructive font-black">*</span>
					</Label>
					<Input
						id="quantity"
						name="quantity"
				...
				<div class="space-y-2">
					...
					<textarea
						...
						name="remarks"
						rows="3"
						placeholder="Reference invoice numbers, supplier names, or delivery notes..."
						bind:value={remarks}
						disabled={loading}
