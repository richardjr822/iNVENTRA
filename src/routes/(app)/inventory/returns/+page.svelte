<script lang="ts">
	...
	import { ArrowLeft, Loader2, RotateCcw } from '@lucide/svelte';
	...
	const selectedProduct = $derived(data.products.find((p) => p.id === product_id));

	function validate() {
		const result = returnsSchema.safeParse({
			product_id,
...
<svelte:head>
	<title>Record Customer Return - Inventra Inventory Management</title>
	<meta name="description" content="Reinstate customer returns back into warehouse stock." />
</svelte:head>
...
<div class="max-w-2xl mx-auto space-y-6">
	...
	<!-- Header -->
	<div>
		<h2 class="text-2xl font-black tracking-tight">Record Returns</h2>
		<p class="text-sm text-muted-foreground">
			Reinstate products returned by customers back to active warehouse stock.
		</p>
	</div>

	<Card.Root class="border-border/40 bg-card/65 backdrop-blur-md shadow-xl">
		<Card.Header class="pb-4">
			<Card.Title class="text-lg font-bold">Return Details</Card.Title>
			<Card.Description class="text-sm">
				Select returned product, enter count, and note the reason or return order number.
			</Card.Description>
		</Card.Header>
		...
		<Card.Content>
			...
			<form method="POST" use:enhance={handleSubmit} class="space-y-5">
				...
				<div class="space-y-2">
					...
					{#if selectedProduct}
						<div
							class="rounded-xl border border-border/40 p-3 bg-muted/20 space-y-1 animate-in fade-in slide-in-from-top-1 duration-150"
						>
				...
				<div class="space-y-2">
					<Label for="quantity" class="text-sm font-bold tracking-wide flex items-center gap-1">
						Quantity Returned <span class="text-destructive font-black">*</span>
					</Label>
					<Input
						id="quantity"
						name="quantity"
				...
				<!-- Remarks Field (Return details) -->
				<div class="space-y-2">
					<Label for="remarks" class="text-sm font-bold tracking-wide"
						>Remarks / Return Reason</Label
					>
					<textarea
						id="remarks"
						name="remarks"
						rows="3"
						placeholder="Reference customer return slips, order numbers, or return reason..."
						bind:value={remarks}
						disabled={loading}
				...
				<div class="flex items-center justify-end gap-3 border-t border-border/40 pt-4">
					...
					<Button
						...
						class="font-bold bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1.5 cursor-pointer text-xs active:scale-95 disabled:scale-100"
					>
						{#if loading}
