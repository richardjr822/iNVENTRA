<script lang="ts">
	...
	$effect(() => {
		if (name && errors.name) {
			const res = createCategorySchema.safeParse({ name, description });
...
<svelte:head>
	<title>Create Category - Inventra Inventory Management</title>
	<meta name="description" content="Add a new physical category to the product taxonomy." />
</svelte:head>
...
<div class="max-w-2xl mx-auto space-y-6">
	...
	<!-- Header titles -->
	<div>
		<h2 class="text-2xl font-black tracking-tight">Create Category</h2>
		<p class="text-sm text-muted-foreground">Add a new category to structure product inventory.</p>
	</div>
	...
	<Card.Root class="border-border/40 bg-card/65 backdrop-blur-md shadow-xl">
		<Card.Header class="pb-4">
			...
			<Card.Description class="text-sm"
				>Define names and descriptive metadata for the new category.</Card.Description
			>
		</Card.Header>
