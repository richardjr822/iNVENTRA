<script lang="ts">
	...
	// Reset error state on keystrokes/selection updates
	$effect(() => {
	...
	$effect(() => {
		...
		if (form?.values) {
			...
			status = (form.values.status as 'active' | 'inactive') || 'active';
			imageUrl = form.values.image_url || null;
		}
	});

	// Client-side file uploading logic with progress bar using XMLHttpRequest
...
<svelte:head>
	<title>Create Product - Inventra Inventory Management</title>
	<meta
		name="description"
		content="Add a new catalog product item with pricing, SKU barcode identifiers, and storage images."
	/>
</svelte:head>
...
<div class="max-w-3xl mx-auto space-y-6">
	...
	<!-- Header titles -->
	<div>
		<h2 class="text-2xl font-black tracking-tight">Create Product</h2>
		<p class="text-sm text-muted-foreground">
			Add a new product item definition to your inventory system.
		</p>
	</div>

	<Card.Root class="border-border/40 bg-card/65 backdrop-blur-md shadow-xl">
		<Card.Header class="pb-4">
			<Card.Title class="text-lg font-bold">Product Details</Card.Title>
			<Card.Description class="text-sm"
				>Fill in identifiers, specifications, categories, and upload image references.</Card.Description
			>
		</Card.Header>
		...
		<Card.Content>
			...
			<form method="POST" use:enhance={handleSubmit} class="space-y-6">
				<!-- Hidden product ID matching generated load UUID -->
							...
							<div class="text-center space-y-2 cursor-pointer">
								...
								<div class="space-y-1">
									<p class="text-xs font-bold text-foreground">
										Drag & drop your product image here, or <span
