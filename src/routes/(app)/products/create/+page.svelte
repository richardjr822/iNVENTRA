<script lang="ts">
	...
	// Variant operations
	function addVariant() {
		// Smart Default for next variant: Quantity empty, Price empty
	...
	// Client-side validation: friendly & simple error messages
	function validate() {
		...
		if (!name.trim()) {
			...
			errors = localErrors;
			// Automatically focus name field
	...
	// Photo file uploading using standard XMLHttpRequest
...
<svelte:head>
	<title>Create Product - Price Monitoring System</title>
</svelte:head>
...
<div class="max-w-[700px] mx-auto space-y-6 pb-20 px-4">
	...
	<!-- Page Header -->
	<div class="space-y-1">
		<h1 class="text-3xl font-extrabold tracking-tight text-foreground">Create Product</h1>
		<p class="text-sm text-muted-foreground font-medium">Add a product and define its selling prices.</p>
	</div>
	...
	<Card.Root class="border border-neutral-200 dark:border-neutral-800 bg-card/90 shadow-sm rounded-[16px] overflow-hidden">
		<Card.Content class="p-6 relative">
			...
			<form method="POST" use:enhance={handleSubmit} class="space-y-6">
				<!-- Hidden fields matching original action handlers -->
