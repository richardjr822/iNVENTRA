<script lang="ts">
	...
	import { Boxes, KeyRound, Loader2, Moon, Sun, User } from '@lucide/svelte';
...
<svelte:head>
	...
	<meta name="description" content="Access the Inventra admin dashboard and manage your inventory, products, and users." />
</svelte:head>

<!-- Premium modern SaaS style background -->
<div class="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-background p-4 md:p-8">
	<!-- Background grid overlay -->
	<div class="absolute inset-0 -z-20 bg-[linear-gradient(to_right,rgba(120,119,198,0.05)_1px,transparent_1px),linear-gradient(to_bottom,rgba(120,119,198,0.05)_1px,transparent_1px)] bg-[size:40px_40px] dark:bg-[linear-gradient(to_right,rgba(255,255,255,0.015)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.015)_1px,transparent_1px)]"></div>
	
	<!-- Background glow blobs -->
	<div class="absolute inset-0 -z-10 overflow-hidden">
		<div class="absolute -top-[40%] left-[20%] h-[60%] w-[60%] rounded-full bg-primary/10 blur-[120px] dark:bg-primary/5"></div>
		<div class="absolute -bottom-[40%] right-[20%] h-[60%] w-[60%] rounded-full bg-emerald-500/10 blur-[120px] dark:bg-emerald-500/5"></div>
	</div>
	...
	{#if mounted}
		<div class="absolute top-4 right-4 z-50">
			...
			>
				{#if isDark}
					<Sun class="h-[1.2rem] w-[1.2rem] text-amber-500" />
				{:else}
					<Moon class="h-[1.2rem] w-[1.2rem] text-slate-700" />
				{/if}
			</Button>
		</div>
	{/if}
	...
	<div class="w-full max-w-md space-y-6">
		...
		<div class="flex flex-col items-center space-y-2 text-center">
			<div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-primary to-emerald-500 text-primary-foreground shadow-lg shadow-emerald-500/10">
				<Boxes class="h-6 w-6" />
			</div>
			<div class="space-y-1">
				<h1 class="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text text-transparent">
					Inventra
				</h1>
				<p class="text-sm font-medium text-muted-foreground">
					Secure Inventory Management Portal
				</p>
			</div>
		</div>

		<!-- Login Card -->
		<Card.Root class="border-border/50 bg-card/60 shadow-2xl backdrop-blur-xl transition-all duration-300 hover:shadow-emerald-500/5 dark:bg-card/40">
			...
			<Card.Content>
				<form method="POST" use:enhance={handleEnhance} class="space-y-4">
					...
					<!-- Username Field -->
					<div class="space-y-2">
						<Label for="username" class="text-sm font-semibold tracking-wide flex items-center gap-1.5">
					...
					<!-- Password Field -->
					<div class="space-y-2">
						<Label for="password" class="text-sm font-semibold tracking-wide flex items-center gap-1.5">
			...
			<Card.Footer class="flex flex-col items-center justify-center border-t border-border/50 py-3 text-center bg-muted/30 rounded-b-xl">
